#!/bin/bash
# run_comparison.sh — Run DV test, then modified TB with same binary, compare traces
#
# Usage:
#   ./run_comparison.sh <test_name> <seed>
#   ./run_comparison.sh riscv_machine_mode_rand_test 12345
#   ./run_comparison.sh riscv_arithmetic_basic_test   # random seed
...
COMPARE_DIR=$WS/comparisons/${TEST_NAME}.${SEED}
mkdir -p "$COMPARE_DIR"
...
# ─────────────────────────────────────────────
# STEP 1: Run DV (original Ibex core)
# ─────────────────────────────────────────────
echo ""
echo ">>> Step 1: Running Ibex DV (original core) ..."

cd "$DV_DIR"
# Remove stale metadata so make doesn't refuse to run
rm -rf out/metadata
...
cp "$DV_TEST_DIR/trace_core_00000000.log"              "$COMPARE_DIR/trace_original.log"
...
# ─────────────────────────────────────────────
# STEP 2: Run modified core TB (same binary)
# ─────────────────────────────────────────────
echo ""
echo ">>> Step 2: Running modified core TB ..."

cd "$WS"
rm -rf xcelium.d   # Clean Xcelium cache
...
awk -F'\t' 'NR>1 && NF>=4 {print $3"\t"$4"\t"$5"\t"$6}' \
    "$COMPARE_DIR/trace_original.log" > "$COMPARE_DIR/stripped_original.log"

awk -F'\t' 'NR>1 && NF>=4 {print $3"\t"$4"\t"$5"\t"$6}' \
    "$MOD_TRACE" > "$COMPARE_DIR/stripped_modified.log"

ORIG_LINES=$(wc -l < "$COMPARE_DIR/stripped_original.log")
MOD_LINES=$(wc -l < "$COMPARE_DIR/stripped_modified.log")

echo "    Original trace:  $ORIG_LINES instructions"
echo "    Modified trace:  $MOD_LINES instructions"

# Use diff to compare (allow different lengths — modified TB may terminate differently)
diff "$COMPARE_DIR/stripped_original.log" \
...
if [ "$DIFF_LINES" -eq 0 ]; then
    ...
    MIN_LINES=$((ORIG_LINES < MOD_LINES ? ORIG_LINES : MOD_LINES))
    head -n "$MIN_LINES" "$COMPARE_DIR/stripped_original.log" > "$COMPARE_DIR/prefix_orig.tmp"
    head -n "$MIN_LINES" "$COMPARE_DIR/stripped_modified.log" > "$COMPARE_DIR/prefix_mod.tmp"
    diff "$COMPARE_DIR/prefix_orig.tmp" "$COMPARE_DIR/prefix_mod.tmp" \
        > "$COMPARE_DIR/prefix_diff.tmp" 2>&1 || true
    PREFIX_DIFF=$(wc -l < "$COMPARE_DIR/prefix_diff.tmp")
    rm -f "$COMPARE_DIR/prefix_orig.tmp" "$COMPARE_DIR/prefix_mod.tmp" "$COMPARE_DIR/prefix_diff.tmp"

    if [ "$PREFIX_DIFF" -eq 0 ]; then
        echo " RESULT: PASS (prefix match)"
        echo " First $MIN_LINES instructions are identical."
        echo " Length difference: original=$ORIG_LINES, modified=$MOD_LINES"
