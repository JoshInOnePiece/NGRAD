module lfsr43 (
  ...
  wire feedback; // cadence syn_keep=1
  wire max_pulse_counter; // cadence syn_keep=1
  reg  [42:0] o_state; // cadence syn_keep=1

  // Example taps for a 43-bit maximal LFSR
  assign feedback = ~(o_state[42] ^
                       o_state[41] ^
                       o_state[37] ^
                       o_state[36]); // cadence syn_keep=1

  counter_5bit ptbCounter ( // cadence syn_keep=1
    .clk(clk),
    .rst_n(rst_n),
...
module counter_5bit (
    ...
    reg [4:0] count;// cadence syn_keep=1
